...
export default function MainLayout() {
  ...
  useEffect(() => {
    if (!enabledFeatures.has('notifications')) return
    const refresh = () => { void notificationSummary().then((value) => setUnreadCount(value.unread_count)).catch(() => {}) }
    refresh(); const timer = window.setInterval(refresh, 30000); window.addEventListener('focus', refresh); window.addEventListener('notification-updated', refresh)
    return () => { window.clearInterval(timer); window.removeEventListener('focus', refresh); window.removeEventListener('notification-updated', refresh) }
  }, [enabledFeatures])
  ...
  const menuItems = useMemo<MenuProps['items']>(() => {
    ...
    if (enabledFeatures.has('admin') && (user?.role === 'admin' || user?.role === 'super_admin')) {
      items.push({
        key: 'admin-group',
    ...
    <Layout style={{ minHeight: '100vh', background: token.colorBgLayout }}>
      ...
      >
        ...
        >
          ...
          {enabledFeatures.has('notifications') && <Badge count={unreadCount} size="small"><Button type="text" icon={<BellOutlined />} onClick={() => navigate('/notifications')} /></Badge>}
        </Header>
