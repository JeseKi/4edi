...
const features = [
  {
    icon: <ThunderboltOutlined className="text-3xl text-[var(--app-link)]" />,
    title: 'FastAPI + React',
    desc: '现代化全栈技术栈，开箱即用',
  },
  {
    icon: <SafetyOutlined className="text-3xl text-[var(--app-link)]" />,
    title: '完整的认证系统',
    desc: 'JWT + TOTP 双因素认证，设备管理中',
  },
  {
    icon: <ToolOutlined className="text-3xl text-[var(--app-link)]" />,
    title: '模块化架构',
    desc: '参考 example_module，快速开发业务模块',
  },
  {
    icon: <GlobalOutlined className="text-3xl text-[var(--app-link)]" />,
    title: '暗色/亮色主题',
    desc: '内置主题切换，Tailwind CSS 4 + Ant Design',
  },
  {
    icon: <MobileOutlined className="text-3xl text-[var(--app-link)]" />,
    title: '响应式设计',
    desc: '移动优先，自适应各种屏幕尺寸',
  },
  {
    icon: <RocketOutlined className="text-3xl text-[var(--app-link)]" />,
    title: '快速开发',
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--app-bg)] text-[var(--app-text-primary)] transition-colors duration-300">
      {/* Header */}
      <header className="fixed top-0 w-full z-50 bg-[var(--app-elevated-bg)] border-b border-[var(--app-border-color)]">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {features.map((feature) => (
              <div
                key={feature.title}
                className="p-6 rounded-xl bg-[var(--app-elevated-bg)] border border-[var(--app-border-color)] theme-card-shadow hover:translate-y-[-4px] transition-all duration-300"
              >
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto text-center">
          <div className="p-10 sm:p-16 rounded-xl bg-[var(--app-elevated-bg)] border border-[var(--app-border-color)]">
            <h2 className="text-3xl sm:text-4xl font-bold mb-4">
