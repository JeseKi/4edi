...
import { canManageUser, isSuperAdmin, resolveErrorMessage } from './utils'
...
export default function UserManagementPage() {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      setSelectedUserIds((prev) => {
        const selectableIds = new Set(data
          .filter((user) => user.id !== currentUser?.id && canManageUser(currentUser?.role, user))
          .map((user) => user.id))
  ...
  }, [currentUser?.id, currentUser?.role, message])
              ...
              {isSuperAdmin(currentUser?.role) && (
                <>
                  <Button
                    ...
                    onClick={() => confirmBulkUpdate({ role: 'super_admin' }, '设为超级管理员')}
                  >
                    设为超级管理员
                  </Button>
                  <Button
        ...
        <Table
          ...
          rowSelection={{
            ...
            getCheckboxProps: (record) => ({
              disabled: record.id === currentUser?.id || !canManageUser(currentUser?.role, record) || isOperating,
            }),
          }}
          loading={loading}
