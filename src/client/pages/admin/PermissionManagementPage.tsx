...
import { canManageUser, getRoleLabel, getRoleTagColor } from './UserManagementPage/utils'
...
export default function PermissionManagementPage() {
  ...
  const columns: TableColumnsType<AdminUser> = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (value: AdminUser['role']) => (
          <Tag color={getRoleTagColor(value)}>{getRoleLabel(value)}</Tag>
        ),
      },
      {
        title: '当前范围',
      ...
      },
      {
        ...
        render: (_, record) => (
          <Button
            ...
            disabled={!canManageUser(currentUser?.role, record) || record.id === currentUser?.id || saving}
          >
            配置权限
          </Button>
        ),
    ...
    [currentUser?.id, currentUser?.role, openEditModal, saving, scopeMap],
  )
      ...
      >
        <Space direction="vertical" size={16} style={{ width: '100%' }}>
          ...
          <Form form={form} layout="vertical" requiredMark={false}>
            <Form.Item label="角色">
              <Typography.Text>{editingUser ? getRoleLabel(editingUser.role) : '-'}</Typography.Text>
            </Form.Item>
            <Form.Item
              label="生效权限"
              name="scopes"
