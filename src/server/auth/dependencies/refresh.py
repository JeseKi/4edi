...
async def get_current_refresh_session(
    ...
    snapshot = await database_executor.run(
        lambda db: _resolve_current_refresh_session(request, db)
    )
    request.state.user_id = snapshot.user_id
    request.state.username = snapshot.username
    request.state.user_role = snapshot.role
    return snapshot


def _resolve_current_refresh_session(
    ...
    return CurrentRefreshSession(
        ...
        refresh_jti=refresh_session.jti,
        payload=payload,
    )
