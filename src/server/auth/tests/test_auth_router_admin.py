...
def test_admin_routes_enforce_admin_scopes(test_client, test_db_session):
    ...
    deleted_user = test_db_session.query(User).filter(User.id == user.id).first()
    assert deleted_user is not None
    assert deleted_user.deleted_at is not None
    remaining_users = test_client.get(
        "/api/admin/users", headers={"Authorization": f"Bearer {write_admin_token}"}
    )
    assert remaining_users.status_code == 200, remaining_users.text
    assert user.id not in {item["id"] for item in remaining_users.json()}


def test_super_admin_can_bulk_update_users(test_client, test_db_session):
    admin = User(
        username="bulk_update_super_admin",
        email="bulk_update_super_admin@example.com",
...
def test_admin_can_bulk_delete_users(test_client, test_db_session):
    ...
    test_db_session.refresh(member_a)
    test_db_session.refresh(member_b)
    assert member_a.deleted_at is not None
...
def test_user_management_role_hierarchy_guards(test_client, test_db_session):
    super_admin = User(
    ...
    admin.set_password("Password123")
    peer_super_admin = User(
        username="peer_super_admin",
        email="peer_super_admin@example.com",
        role=UserRole.SUPER_ADMIN,
    )
    peer_super_admin.set_password("Password123")
    member = User(
        username="hierarchy_member",
        email="hierarchy_member@example.com",
        role=UserRole.USER,
    )
    member.set_password("Password123")
    test_db_session.add_all([super_admin, admin, peer_super_admin, member])
    test_db_session.commit()
    test_db_session.refresh(super_admin)
    test_db_session.refresh(admin)
    test_db_session.refresh(peer_super_admin)
    test_db_session.refresh(member)

    admin_token = service.create_access_token(
        {"sub": admin.username, "scope": service.get_user_scopes(admin)}
    )
    super_token = service.create_access_token(
        {"sub": super_admin.username, "scope": service.get_user_scopes(super_admin)}
    )
    admin_headers = {"Authorization": f"Bearer {admin_token}"}
    super_headers = {"Authorization": f"Bearer {super_token}"}

    blocked_update = test_client.patch(
        f"/api/admin/users/{peer_super_admin.id}",
        json={"name": "blocked"},
        headers=admin_headers,
    )
    assert blocked_update.status_code == 403

    blocked_promotion = test_client.patch(
    ...
    blocked_scope_update = test_client.put(
        f"/api/admin/users/{peer_super_admin.id}/scopes",
        json={"scopes": [service.SCOPE_PROFILE_READ]},
        headers=admin_headers,
    )
    assert blocked_scope_update.status_code == 403

    blocked_delete = test_client.delete(
        f"/api/admin/users/{peer_super_admin.id}",
        headers=admin_headers,
    )
    ...
    blocked_create = test_client.post(
        "/api/admin/users",
        json={
            "username": "blocked_elevated_user",
            "email": "blocked-elevated@example.com",
    ...
    blocked_bulk = test_client.patch(
        "/api/admin/users/bulk",
        json={"user_ids": [member.id, peer_super_admin.id], "status": "inactive"},
        headers=admin_headers,
    )
    assert blocked_bulk.status_code == 403
    test_db_session.refresh(member)
    assert member.status == UserStatus.ACTIVE

    member_update = test_client.patch(
        f"/api/admin/users/{member.id}",
    ...
    super_update = test_client.patch(
        f"/api/admin/users/{admin.id}",
        json={"name": "managed by super admin"},
        headers=super_headers,
    )
    assert super_update.status_code == 200, super_update.text

    super_promotion = test_client.patch(
        f"/api/admin/users/{member.id}",
        json={"role": "super_admin"},
        headers=super_headers,
    )
    assert super_promotion.status_code == 200, super_promotion.text
    assert super_promotion.json()["role"] == UserRole.SUPER_ADMIN.value

    self_role_update = test_client.patch(
        f"/api/admin/users/{super_admin.id}",
        json={"role": "admin"},
        headers=super_headers,
    )
    assert self_role_update.status_code == 400
