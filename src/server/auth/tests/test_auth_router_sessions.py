...
def test_admin_soft_delete_revokes_sessions_and_hides_user(test_client, test_db_session: Session):
    admin = User(
        username="soft_delete_admin",
        email="soft_delete_admin@example.com",
        role=UserRole.ADMIN,
    )
    admin.set_password("Password123")
    member = User(username="soft_delete_member", email="soft_delete_member@example.com")
    ...
    member_access_token = member_login.json()["access_token"]
    member_refresh_token = member_login.cookies.get("fullstack_template_refresh_token")
    assert member_refresh_token is not None
    ...
    assert test_client.get(
        "/api/auth/profile", headers={"Authorization": f"Bearer {member_access_token}"}
    ).status_code == 401

    test_client.cookies.set("fullstack_template_refresh_token", member_refresh_token)
    assert test_client.post("/api/auth/refresh").status_code == 401

    users_response = test_client.get(
        "/api/admin/users", headers={"Authorization": f"Bearer {admin_token}"}
    )
    assert users_response.status_code == 200, users_response.text
    assert member.id not in {item["id"] for item in users_response.json()}
