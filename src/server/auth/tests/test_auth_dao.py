...
def test_user_dao_soft_delete_hides_but_preserves_user(test_db_session: Session):
    """软删除用户后，默认查询不可见但历史记录仍保留。"""
    ...
    assert dao.get_by_username(user.username) is None
    assert dao.get_by_email(user.email) is None
    assert dao.list_all() == []
    deleted_user = dao.get_by_id(user.id, include_deleted=True)
