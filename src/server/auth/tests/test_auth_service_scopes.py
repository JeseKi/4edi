...
def test_super_admin_has_the_full_administrator_scope_range():
    assert get_role_scopes(UserRole.SUPER_ADMIN) == get_role_scopes(UserRole.ADMIN)


def test_assignable_scopes_default_to_role_scopes():
    assert get_assignable_role_scopes(UserRole.USER) == get_role_scopes(UserRole.USER)
