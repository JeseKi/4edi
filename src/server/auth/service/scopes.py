...
SCOPE_DEFINITIONS: tuple[ScopeDefinition, ...] = (
    ...
    ScopeDefinition(
        scope=SCOPE_ADMIN_AUDIT_READ,
        title="查看审计日志",
        description="查看系统写操作、任务执行和安全事件审计日志。",
    ),
    ScopeDefinition(
        scope=SCOPE_FILES_READ,
        title="查看文件",
        description="查看和下载自己创建的私有文件；管理员可访问全部文件。",
    ),
    ScopeDefinition(
        scope=SCOPE_FILES_WRITE,
        title="管理文件",
        description="创建上传意图、确认上传和删除自己创建的文件。",
    ),
    ScopeDefinition(scope=SCOPE_NOTIFICATIONS_READ, title="查看站内消息", description="查看自己的站内消息并更新已读状态。"),
    ScopeDefinition(scope=SCOPE_ADMIN_NOTIFICATIONS_WRITE, title="发布站内消息", description="向单个用户或全体活跃用户发布站内消息。"),
)
...
ROLE_SCOPES: dict[UserRole, tuple[str, ...]] = {
    ...
    UserRole.ADMIN: ADMIN_ROLE_SCOPES,
    UserRole.SUPER_ADMIN: ADMIN_ROLE_SCOPES,
}

# 默认情况下可分配范围与角色默认范围相同。业务应用可在此处为指定角色
# 增加按用户授予、但不默认拥有的权限；读取与校验必须使用同一集合。
ROLE_ASSIGNABLE_SCOPES: dict[UserRole, tuple[str, ...]] = dict(ROLE_SCOPES)
...
def get_assignable_role_scopes(role: UserRole) -> tuple[str, ...]:
...
def get_user_scope_overrides(user: User) -> tuple[str, ...] | None:
    ...
    allowed_scopes = set(get_assignable_role_scopes(user.role))
...
def validate_scope_overrides(
    ...
    allowed_scopes = set(get_assignable_role_scopes(role))
    ...
    return tuple(
        scope for scope in get_assignable_role_scopes(role) if scope in set(normalized)
    )
