...
在 `config/system.toml` 的 `[providers]` 中指定需要 mock 的 provider：
...
6. 如果需要真实配置，在 `config/system.toml`、对应 settings 类和 `.env.example`（仅密钥）增加配置项。
...
- 不要在业务模块中直接判断 mock provider 配置，mock/real 选择只应存在于 provider registry。
