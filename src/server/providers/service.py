...
def sync_external_providers(db: Session) -> None:
    ...
    mock_keys = set(global_config.providers.resolved_mock_list(global_config.app.env))
...
def _mode_for(key: str) -> ProviderImplementation:
    if key in global_config.providers.resolved_mock_list(global_config.app.env):
        return "mock"
