...
def is_provider_enabled(provider: str) -> bool:
    return provider.upper() in global_config.oauth.enabled_providers
...
def _resolve_provider_user(
    ...
    *,
    ...
    if user is None:
        if user_dao.get_by_email(provider_email, include_deleted=True) is not None:
            raise OAuthError("该邮箱对应的账号已被删除")
...
def _make_unique_username(db: Session, candidate: str, fallback_prefix: str) -> str:
    ...
    user_dao = UserDAO(db)
    if user_dao.get_by_username(normalized, include_deleted=True) is None:
        return normalized
    ...
    for _ in range(10):
        ...
        if user_dao.get_by_username(username, include_deleted=True) is None:
            return username
