...
def test_oauth_providers_follow_oauth_list(test_client, monkeypatch):
    monkeypatch.setattr(global_config.oauth, "enabled_providers", [])
    ...
    monkeypatch.setattr(global_config.oauth, "enabled_providers", ["GITHUB", "GOOGLE"])
    resp = test_client.get("/api/oauth/providers")
    assert resp.status_code == 200, resp.text
    assert resp.json() == {
        "providers": [
...
def test_google_oauth_auto_creates_user_and_ticket(
    ...
    monkeypatch.setattr(global_config.oauth, "enabled_providers", ["GOOGLE"])
