...
class DeviceCodePollError:
    """需要提交轮询状态后再返回给客户端的 OAuth 协议错误。"""
    ...
    *,
    ...
    if not approve:
        ...
        return {
            ...
            "scopes": list(auth_service.normalize_scopes(device_code.scope)),
        }
    ...
    *,
    ...
    if code_record.approved_at is None:
        if _is_polling_too_fast(code_record, now):
            ...
            return DeviceCodePollError("slow_down")
        dao.update(code_record, last_polled_at=now)
        return DeviceCodePollError("authorization_pending")
...
def _build_verification_uri() -> str:
    base_url = global_config.app.domain.strip().rstrip("/")
